...
class SocketWrapper
{
     ...
     * @throws   MessageException()     In case of timeout
     */
     ...
     * @param int|null $localTimeout timeout value to use just for this call (used when waiting for "100 Continue" response)
     * @return string Available data up to the newline (not including newline)
     * @throws MessageException
     */
    public function readLine(int $bufferSize, int $localTimeout = null)
    {
        $line = '';
        while (!feof($this->socket)) {
            ...
            if (null === $localTimeout) {
                ...
                throw new MessageException(
                    "readLine() call timed out", Request2Exception::TIMEOUT
                );
            }
            if (substr($line, -1) == "\n") {
                return rtrim($line, "\r\n");
    ...
    public function write(string $data)
    {
        ...
        while (strlen($data)) {
            ...
            if (0 === (int)$written) {
                throw new MessageException('Error writing request');
            }
    ...
    protected function checkTimeout()
    {
        ...
        if ($info['timed_out'] || $this->deadline && microtime(true) > $this->deadline) {
            ...
            throw new MessageException(
                "Request timed out {$reason}", Request2Exception::TIMEOUT
            );
        }
    }
     ...
     *
     * @return array First element is $tv_sec parameter for stream_select(), second element is $tv_usec
     */
     ...
     * the message of ConnectionException
     *
     * @param int $errno  error level
     * @param string $errstr error message
     * @return bool
     */
    protected function connectionWarningsHandler(int $errno, string $errstr)
    {
        if ($errno & E_WARNING) {
