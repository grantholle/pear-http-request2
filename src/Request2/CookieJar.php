...
class CookieJar implements \Serializable
{
     ...
     *
     * @param array $cookie cookie data, as returned by {@link Response::getCookies()}
     * @param Url2|null $setter URL of the document that sent Set-Cookie header
     * @return array Updated cookie array
     * @throws LogicException
     * @throws MessageException
     */
    protected function checkAndUpdateFields(array $cookie, Url2 $setter = null)
    {
        ...
        if (preg_match(Request2::REGEXP_INVALID_COOKIE, $cookie['name'])) {
            throw new LogicException(
                "Invalid cookie name: '{$cookie['name']}'",
                Request2Exception::INVALID_ARGUMENT
            );
        }
        if (preg_match(Request2::REGEXP_INVALID_COOKIE, $cookie['value'])) {
            throw new LogicException(
     ...
     * @throws MessageException
     */
    public function store(array $cookie, Url2 $setter = null)
    {
        try {
     ...
     * @param Url2|null $setter original request URL, needed for setting default domain/path. If not given, effective URL from response will be used.
     * @return bool whether all cookies were successfully stored
     * @throws LogicException
     * @throws Request2Exception
     */
    public function addCookiesFromResponse(Response $response, Url2 $setter = null)
    {
        if (null === $setter) {
     ...
     * @return string|bool Registered domain, will return false if $domain is either invalid or a TLD itself
     */
