...
use Pear\Http\Request2\SocketWrapper;
use Pear\Http\Request2\SOCKS5;
use Pear\Net\Url2;

/**
 * Socket-based adapter for Request2
 *
 * PHP version 5
 *
 * LICENSE
...
class Socket extends Adapter
{
    ...
    protected function connect()
    {
        ...
        $tunnel  = Request2::METHOD_CONNECT == $this->request->getMethod();
        ...
        if (!($host = $this->request->getConfig('proxy_host'))) {
            ...
            if (!($port = $this->request->getConfig('proxy_port'))) {
                throw new Request2_LogicException(
                    'Proxy port not provided',
                    Request2Exception::MISSING_VALUE
                );
            }
    ...
    {
        $donor   = new self;
        $connect = new Request2(
            $this->request->getUrl(), Request2::METHOD_CONNECT,
        ...
        if (200 > $response->getStatus() || 300 <= $response->getStatus()) {
            throw new Request2_ConnectionException(
     ...
     * @return   boolean
     */
    protected function canKeepAlive($requestKeepAlive, Response $response)
    {
        // Do not close socket on successful CONNECT request
        if (Request2::METHOD_CONNECT == $this->request->getMethod()
        ...
        $lengthKnown = 'chunked' == strtolower($response->getHeader('transfer-encoding'))
                       ...
                       || Request2::METHOD_HEAD == $this->request->getMethod()
     ...
     *
     * @param Request2          $request  Original request
    ...
    protected function handleRedirect(
        Request2 $request, Response $response
    ) {
        ...
        if (0 == $this->redirectCountdown) {
            ...
            throw new MessageException(
                ...
                Request2Exception::TOO_MANY_REDIRECTS
            );
        }
        $redirectUrl = new Url2(
            $response->getHeader('location'),
            [Url2::OPTION_USE_BRACKETS => $request->getConfig('use_brackets')]
        );
        ...
        if ($redirectUrl->isAbsolute()
            ...
            throw new MessageException(
                ...
                Request2Exception::NON_HTTP_REDIRECT
            );
        }
        ...
        if (303 == $response->getStatus()
            ...
            $redirect->setMethod(Request2::METHOD_GET);
            $redirect->setBody('');
        }
     ...
     *
     * @return bool whether another request should be performed
     */
    protected function shouldUseServerDigestAuth(Response $response)
    {
     ...
     * @throws   Request2Exception in case of unsupported challenge parameters
     */
    protected function shouldUseProxyDigestAuth(Response $response)
    {
     ...
     *
     * Request2 follows IE and Firefox (and hopefully RFC 2617) in
     ...
     * @throws   Request2_NotImplementedException in case of unsupported challenge parameters
     */
    ...
    protected function addProxyAuthorizationHeader(&$headers, $requestUrl)
    {
        if (!$this->request->getConfig('proxy_host')
            ...
            || (0 == strcasecmp('https', $this->request->getUrl()->getScheme())
                && Request2::METHOD_CONNECT != $this->request->getMethod())
        ) {
            return;
        }
    ...
    protected function prepareHeaders()
    {
        $headers = $this->request->getHeaders();
        $url     = $this->request->getUrl();
        $connect = Request2::METHOD_CONNECT == $this->request->getMethod();
        $host    = $url->getHost();
     ...
     *
     * @throws Request2_LogicException
            ...
            while ($pos < $length) {
                ...
                if (',' === substr($headers['expect'], $pos, 1)) {
                    ...
                    throw new Request2_LogicException(
                        ...
                        Request2Exception::INVALID_ARGUMENT
                    );
    ...
    protected function readResponse()
    {
        ...
        do {
            try {
                ...
                if (Request2Exception::TIMEOUT === $e->getCode()
        ...
        if (Request2::METHOD_HEAD == $this->request->getMethod()
            || (Request2::METHOD_CONNECT == $this->request->getMethod()
                && 200 <= $response->getStatus() && 300 > $response->getStatus())
            || in_array($response->getStatus(), [204, 304])
        ) {
        ...
        if (0 == $this->chunkLength) {
            ...
            if ('' === $line && $this->socket->eof()) {
                ...
                throw new MessageException(
                    ...
                    Request2Exception::DECODE_ERROR
                );
