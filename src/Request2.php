...
use Pear\Http\Request2\Adapters\Adapter;
...
// pear-package-only /**
// pear-package-only  * Exception class for Request2 package
// pear-package-only  */
...
class Request2 implements SplSubject
{
     ...
     *
     * @param string|Url2|null $url Request URL
     ...
     * that will be set to '/'.
     *
     * @param string|Url2 $url Request URL
     * @return Request2
     * @throws LogicException
     */
    public function setUrl($url)
    {
        if (is_string($url)) {
            $url = new Url2(
                $url, [Url2::OPTION_USE_BRACKETS => $this->config['use_brackets']]
            );
        }
     ...
     *
     * @return Request2
     * @throws LogicException if the method name is invalid
     */
     ...
     *   <li> 'proxy_auth_scheme' - Proxy auth scheme, one of Request2::AUTH_* constants (string)</li>
     ...
     * @return mixed value of $name parameter, array of all configuration parameters if $name is not given
     * @throws LogicException
     */
     ...
     * @return Request2
     */
    public function setAuth(string $user = '', string $password = '', string $scheme = self::AUTH_BASIC)
    {
        $this->auth = empty($user)
            ? null
            : [
                'user'     => (string)$user,
     ...
     * is one of the Request2::AUTH_* constants.
     *
     * @return   array
     */
     ...
     * @return Request2
     * @throws LogicException
     * @throws Request2\Exceptions\MessageException
     * @see setCookieJar()
     */
     ...
     *
     * @param string|resource|MultipartBody $body       Either a
     ...
     *
     * @return string|resource|MultipartBody
     */
     ...
     *                                   body, data is Response object</li>
     ...
     *
     * @param CookieJar|bool $jar Existing CookieJar object, true to create a new one, false to remove
     * @return Request2
     * @throws LogicException
     */
     ...
     *
     * @return   Response
     *@throws   Request2Exception|Request2\Exceptions\Exception
     */
    public function send()
    {
        // Sanity check for URL
        if (!$this->url instanceof Url2
            || !$this->url->isAbsolute()
            || !in_array(strtolower($this->url->getScheme()), ['https', 'http'])
        ) {
            throw new LogicException(
                'Request2 needs an absolute HTTP(S) request URL, '
                . ($this->url instanceof Url2
     ...
     * @param bool $detectType whether to try autodetecting MIME type of file, will only work if $file is a filename, not pointer
    ...
    protected function fopenWrapper($file, $detectType = false)
    {
        ...
        if (is_string($file)) {
            if (!($fileData['fp'] = @fopen($file, 'rb'))) {
                ...
                throw new LogicException(
                    $error['message'], Request2Exception::READ_ERROR
                );
            }
            if ($detectType) {
        ...
        if (!($stat = fstat($fileData['fp']))) {
            throw new LogicException(
                "fstat() call failed", Request2Exception::READ_ERROR
            );
        }
